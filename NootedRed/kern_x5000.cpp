    ...
    if ((engineType == 1 || engineType == 2) && NRed::callback->chipType >= ChipType::Renoir) {
        ...
        while (i < 0x80) {
            ...
            if (op == 0) {    // SDMA_OP_NOP
                ...
                auto ibBuf = reinterpret_cast<uint32_t *>(IONew(uint32_t, ibSize));
                auto ibBufDst = reinterpret_cast<uint8_t *>(ibBuf);
                auto sizeLeft = ibSize * 4;
                while (sizeLeft != 0) {
                    uint32_t bytesToCopy = min(sizeLeft, 0x1000 - (ibPtr & 0xFFF));
                    uint64_t translated = translateVA(ibPtr, 0, eAMD_VM_HUB_TYPE::MM);
                    DBGLOG("x5000", "writeTail: IB's VA translated to 0x%llX", translated);

                    auto *memDesc = IOGeneralMemoryDescriptor::withPhysicalAddress(
                        static_cast<IOPhysicalAddress>(translated), bytesToCopy, kIODirectionIn);
                    auto *map = memDesc->map();
                    auto mapped = reinterpret_cast<uint8_t *>(map->getVirtualAddress());

                    memcpy(ibBufDst, mapped, bytesToCopy);
                    map->unmap();
                    map->release();
                    memDesc->release();

                    sizeLeft -= bytesToCopy;
                    ibBufDst += bytesToCopy;
                    ibPtr += bytesToCopy;
                }
...
void X5000::executeSDMACopyLinear(uint32_t byteCount, uint64_t srcOffset, uint64_t dstOffset, uint8_t vmid) {
    ...
    while (byteCount != 0) {
        uint32_t toWrite = min(byteCount, 0x1000 - max(srcOffset & 0xFFF, dstOffset & 0xFFF));
        ...
        if (dstIsVA) {
            ...
            DBGLOG("x5000", "executeSDMACopyLinear: dstOffset VA 0x%llX translated to 0x%llX", dstOffset, dst);
            ;
        }
...
void X5000::executeSDMAPollRegmem(bool memPoll, uint64_t addr, uint32_t ref, uint32_t mask, uint16_t retryCount,
    ...
    if (memPoll) {
        if (isVRAMAddress(addr)) {
            ...
            DBGLOG("x5000", "executeSDMAPollRegmem: addr VA translated to 0x%llX", addr);
        }
...
void X5000::executeSDMAConstFill(uint8_t fillSize, uint32_t srcData, uint64_t dstOffset, uint32_t byteCount,
    ...
    while (byteCount != 0) {
        uint32_t toWrite = min(byteCount, 0x1000 - (dstOffset & 0xFFF));
        uint64_t dst = dstOffset;
        dstOffset += toWrite;
        byteCount -= toWrite;
        ...
        if (isVA) {
            dst = translateVA(dst, vmid, eAMD_VM_HUB_TYPE::MM);
            DBGLOG("x5000", "executeSDMAConstFill: dstOffset VA 0x%llX translated to 0x%llX", dstOffset, dst);
        }
